...
OutputExcel=['Tuning_output\Master_tuning_output_TLT' Todaystamp '.xlsx'];
%% Get data & Setting
...
load('C:\Users\gly19\Dropbox\GU\1.Investment\4. Alphas (new)\42. Sigmaa003_Enhanced\Data\TLTOptionData.mat');


%% Load BMK
OutputCollection_BMK = BMK(tlt_price,Datestamp_tlt,'TLT');
OutputCollection_TY = BMK(ty_prices,Datestamp_tlt,'TY10');
...
for i=1:size(TradeOptionType_range,2)
    ...
    if strcmp(TradeOptionType,'1stM')
        Ivolcurve=Ivolcurve_1stM_tlt;
    elseif strcmp(TradeOptionType,'2ndM')
        Ivolcurve=Ivolcurve_2ndM_tlt;
    end
        ...
        for k=1:size(dynamic_weight_type_range,2)
           ...
           % Below is the default numbers
            ...
            switch dynamic_weight_type
                ...
                case 'FixedLeverage1' %
                    Vix_Thld=[13 20 50]; Leverge_step=[1 1 1];
                case 'FixedLeverage1.5' %
                    Vix_Thld=[13 20 50]; Leverge_step=[1.5 1.5 1.5];
                case 'FixedLeverage1.8' %
                    Vix_Thld=[13 20 50]; Leverge_step=[1.8 1.8 1.8];
                case 'FixedLeverage2.5' %
                    Vix_Thld=[13 20 50]; Leverge_step=[2.5 2.5 2.5];
                case 'YueDieYueSell210' %
                    Vix_Thld=[13 20 50]; Leverge_step=[2 1 1];
                case 'YueDieYueSell1.510' %
                    Vix_Thld=[13 20 50]; Leverge_step=[1.5 1 1];
                case 'YueDieYueSell21.50' %
                    Vix_Thld=[13 20 50]; Leverge_step=[2 1.5 1.5];
                case 'YueDieYueSell2.51.80' %
                    Vix_Thld=[13 20 50]; Leverge_step=[2.5 1.8 1.8];
                case 'YueDieYueSell320' %
                    Vix_Thld=[13 20 50]; Leverge_step=[3 2 2];
                otherwise
            ...
            % Main line
            [OutputCollection] = sigmaa003_2023(tlt_price,Ivolcurve_1stM_tlt,Ivolcurve_2ndM_tlt,Vix_dat_tlt,ExpiryDate_tlt,Datestamp_tlt,TradeOptionType,OptionMoneyness,dynamic_weight,Vix_Thld,Leverge_step);
            ...
            save(['Tuning_output\Master_tuning_output_TLT_' Todaystamp '.mat'],"Master_tuning_output","-mat");
...
CollectionCumpnl=array2timetable(CollectionCumpnl,"RowTimes",Datestamp_tlt,'VariableNames',CollectionRowName);
